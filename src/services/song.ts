...
export async function getSongData(songId: string): Promise<SongData> {
  return CacheService.getOrFetch<SongData>(`song_${songId}`, async () => {
    const scraper = new MelonScraper();
    const path = `/song/detail.htm?songId=${songId}`;

    try {
      // Use the scraper to fetch HTML instead of direct fetch
      const $ = await scraper.fetchHtml(path);
      ...
      $(".section_prdcr .list_person li").each((i, el) => {
        const name = $(el).find(".ellipsis.artist .artist_name").text().trim();
      ...
      if (error instanceof Error) {
        throw new Error(`Failed to fetch song data: ${error.message}`);
      }
      throw new Error("Unknown error occurred while fetching song data");
    }
  });
}
