...
export async function getAlbumData(albumId: string): Promise<AlbumData> {
  return CacheService.getOrFetch(`album_${albumId}`, async () => {
    ...
    try {
      ...
      let title = "";
      const songNameDiv = $(".section_info .wrap_info .entry .info .song_name");
      ...
      throw new Error("Unknown error occurred while fetching album data");
    }
  });
}
